...
| `operations[].schedule` | disparo y efecto observable + —si su efecto SALE de la transacción (activación `via: {client, call}` o correo), o si la raíz no lleva bloqueo optimista— el de **clúster**: dos instancias vivas, VARIAS filas candidatas y un `Then` que cuenta el efecto externo **recibido** y afirma una vez por fila. El escenario tiene que **nombrar la operación**. Publicando un evento NO se pide: la transacción ya arbitra, y el escenario no podría ponerse rojo |
...
**Tres de ellas se escriben primero en el YAML**, porque cambian el código generado y el generador no lee esta sección: ausencia vs nulo → `conventions.nulls` en el manifiesto; escala que se valida o se redondea → `constraints.scalePolicy` donde vive la escala (el value type); mayúsculas y acentos → `compare` en el campo `unique` y `match` + `compare` en el filtro de la query. Aquí se **recuerdan** en prosa para quien lee los escenarios, nunca se declaran solo aquí: la corrida `catalog` las tenía solo en esta sección y el agente reescribió 85 archivos de build para cumplirlas. `keel validate` avisa (`CHK-SCEN-CONVENTION-UNBACKED`) cuando la prosa dice una que el YAML no declara.
...
- **Barrido cuyo efecto sale de la transacción, sin escenario de clúster**: `@Scheduled` corre en TODAS las réplicas, así que sin reclamo las N se llevan las mismas filas y cada una llama al proveedor — y la llamada del perdedor ya está en el cable cuando su transacción hace rollback. Es el hueco que ningún escenario de una sola instancia puede tapar. Publicando un evento el hueco no existe (la transacción y el `@Version` arbitran), y pedirlo ahí produce un escenario que no puede fallar: se midió en la corrida `refunds-http`, rompiendo el reclamo a propósito y viendo los dos escenarios seguir verdes.
