...
Tres de ellas cambian el código generado y por eso se DECLARAN en el YAML, y aquí solo se
recuerdan: ausencia vs nulo (`conventions.nulls` del manifiesto), qué pasa con los decimales
de más (`constraints.scalePolicy`) y qué es el mismo texto (`compare`/`match`). Escritas solo
aquí, el generador no las ve y `keel validate` avisa.>
...
Casi todo lo de § Reglas de cobertura es revisión de `/keel-validate`, que lee y juzga. Veinte reglas están además **mecanizadas** en `keel validate`, y se reparten en tres grupos.

**Las que cruzan este documento con el DISEÑO** (`CHK-SCEN-*`), que es lo que impide que la matriz de cobertura la escriba y la verifique el mismo agente: toda operación tiene su fila; ninguna fila nombra una operación que no existe; ningún flujo citado en la matriz falta como escenario; todo `code` declarado aparece en algún escenario (una vez por code, no por operación que lo declare); y todo estado del `lifecycle` lo nombra alguien. Las filas transversales que el formato admite —`**clúster (2 réplicas)**`, que agrupa por mecanismo— se ignoran a propósito: leerlas como operaciones convertiría una tabla bien escrita en una lista de falsos hallazgos.

**Las que buscan la señal de un MECANISMO**, que busca las señales en el texto de este archivo y **avisa** cuando no las encuentra: los dos escenarios de repetición de cada compensación (reentrega y doble entrega simultánea), **la reentrega de cualquier otra suscripción con guarda declarada**, el escenario de carrera de cada operación con `idempotency`, los DOS escenarios del outbox cuando el diseño declara `reliability: outbox` —el del canal indisponible y el del evento abandonado—, la espera agotada de cada `reconciledBy`, el rescate de lo que otra réplica dejó en vuelo, **el escenario de clúster de cada barrido cuyo efecto sale de la transacción**, los `serviceClient` y los roles que los escenarios nombran y el diseño no declara, y el barrido que no tiene ni puerta ni efecto declarado contra el que afirmar.

**Las que contrastan lo que un `Then` AFIRMA con lo que el YAML DECLARA**, que nacieron de la corrida `catalog` (2026-09-21) —cuatro huecos que encontró un agente con la suite ya escrita y ninguno `keel validate`—: un `Then` que **cuenta** las operaciones bajo una ruta y la cuenta no casa con `api` (`CHK-SCEN-OP-COUNT`); un `Then` que **enumera** el payload de un evento y se deja campos que `messaging` declara, que con la convención de ausencia equivale a afirmar que no viajan (`CHK-SCEN-EVENT-PAYLOAD-PARTIAL`; solo cuando el ítem nombra al menos la mitad de los campos: nombrar dos no es enumerar); un `Then` que afirma **quién va primero** en un listado ordenado por `updatedAt` cuando el Given mueve filas de estado tras crearlas sin decir cuál fue la última escritura (`CHK-SCEN-ORDER-BY-MUTATED`); y una **convención de determinación** dicha en § Convenciones que el YAML no declara o contradice (`CHK-SCEN-CONVENTION-UNBACKED`: ausencia vs nulo, escala que se valida y no se ajusta, mayúsculas y acentos). Las cuatro tienen la misma raíz: el documento dice algo que el diseño permite comprobar, y lo encontraba el sitio más caro — un agente con la suite escrita, desde donde la tentación es corregir el escenario para que pase.

**Y dos puertas más que no son lectura de este archivo, sino de lo que lo rodea.** El **careo de flujos** (`CHK-SCEN-FLOW-REVIEW-STALE`): un subagente de contexto limpio, `keel-flow-review`, ejecuta cada `FL-*` paso a paso contra el YAML —lo que ninguna regex ve: el estado que un paso deja al siguiente— y deja sus contradicciones en `flow-review.yaml`, sellado con el texto de este documento; `keel validate` avisa mientras falte, esté caducado o tenga hallazgos sin decidir, y dice **qué flujos** recarear: el sello es por bloque, no del documento entero. El careo tiene **presupuesto —tres pasadas por versión del diseño—** y termina de una de dos formas, nunca de otra: sin hallazgos abiertos, o con el presupuesto agotado y lo que queda DECIDIDO (`CHK-SCEN-FLOW-REVIEW-EXHAUSTED`). No converge solo —cada pasada encuentra algo nuevo—, y un gate que no termina se aprende a ignorar. Y la **coherencia de los contratos derivados** (`CHK-DOCS-*`): `openapi.yaml`, `asyncapi.yaml` y la colección Postman contrastados contra el diseño en su contenido —rutas, status, canales, campos, una carpeta por flujo y cada request con un status que su endpoint puede dar—, no solo en su versión.
...
- **Y la única forma de saberlo es romper el mecanismo y mirar.** Que la suite salga al 100% dice que ningún escenario falla, no que alguno pueda fallar; la diferencia no se ve en el verde. Se mide sobre el servidor ya generado, mutando **un** mecanismo cada vez de forma que el código siga compilando —la guarda de reentrega que responde siempre «no visto», el reclamo del barrido que pierde su condición de partida— y comprobando que el escenario que lo cubre se pone rojo. Si no se pone, ese mecanismo no lo prueba ningún escenario: en una corrida real se rompió el reclamo del barrido y los dos escenarios que lo cubrían siguieron verdes. Es trabajo del cierre de la generación y no del diseño, pero el resultado sí vuelve aquí: un mecanismo sin escenario que lo mida es un escenario que falta escribir.
