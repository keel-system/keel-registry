...
La versión vigente es la última de la tabla, y es la que siembra `keel new` en el manifiesto; `keel validate` acepta todas las anteriores porque cada entrada es aditiva. Los `$id` de los schemas siguen apuntando a `keel.dev/schema/2.0/`: son **URIs de identidad** con los que los schemas se referencian entre sí, no la versión del DSL, y por eso no se tocan al versionar.
