...
- **Cuándo el cuerpo HTTP es obligatorio: lo decide `required`, y no se declara aparte.** Si ningún campo del `input` que viaja en el cuerpo es `required`, el cuerpo **entero** es opcional: una petición sin cuerpo es válida y no puede responder 400 (`retireProduct` con un único `reason` opcional se acepta sin cuerpo). Basta un campo `required: true` para que el cuerpo lo sea. Es derivable, así que no hay nada que declarar — pero sí que **escribir en el escenario**: el `When` dice si manda cuerpo o no, y dejarlo al criterio de quien escribe la prueba fue uno de los huecos que reportó la corrida `catalog`.
- **Un `input: { fields: … }` NO hereda del dominio.** `{ entity: X }` sí: toma los campos de la entidad con sus `constraints`. Con `fields` el nodo se toma tal cual, así que redeclarar `subject: { type: string, required: true }` cuando `Template.subject` declara `maxLength: 200` deja la entrada **sin esa cota** — y con un campo `list`, redeclarar el tipo pierde además el `maxItems` y el value type entero. No es un defecto: el input y el almacén son **contratos distintos**, y una operación puede aceptar a propósito algo distinto de lo que guarda. Lo que no puede es que la diferencia sea invisible, porque el precio se paga lejos: sin la cota en el input, la violación no se descubre como **400 en el borde** sino como **conflicto de integridad al escribir**, y el `code` que recibe el cliente no es el que el diseño declaró. Por eso `keel validate` **avisa** cuando un campo del input se llama igual que un campo acotado del dominio y no declara cota: se cierra declarándola (o dándole el value type que ya la lleva), no aceptándola por escrito. Y no se hereda por nombre a propósito: eso sería inventar un enlace que el diseño no declara.
...
- **El DTO de una entidad hija lleva SIEMPRE el id de su raíz** (`<raíz>Id`), aunque la proyección declarada no lo enumere: es el mismo tipo cuando la hija se devuelve suelta —donde el id del padre es imprescindible— que cuando va anidada, y partirlo en dos formas haría que el mismo objeto tuviera dos contratos según por dónde salga. Anidado es una redundancia, no una fuga: el padre ya está ahí. Un `exclude` con dot-path sobre la relación (`exclude: [images.productId]`) **no** lo quita, y el generador lo avisa en vez de obedecer en silencio.
...
- Un **filtro de texto** en el `input` de una query declara cómo casa: `match` (`exact` por defecto, `contains` o `prefix`) y `compare` (`exact`, `ignore-case`, `ignore-case-accents`). `name: { type: string, match: contains, compare: ignore-case-accents }` es «coincidencia parcial ignorando mayúsculas y acentos» — escrito donde el generador lo ve, en vez de en una `rule`. `match` solo vale en el input de una query; `compare` y `match` solo sobre texto.
